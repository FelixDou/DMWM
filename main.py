...
from models import ActorModel, Encoder, ObservationModel, RewardModel, TransitionModel, ValueModel, bottle, NLR
from utils import FreezeParameters, imagine_ahead, lambda_return, lineplot, write_video

# Hyperparameters
parser = argparse.ArgumentParser(description='Dual Mind World Model')
...
parser.add_argument('--logic-vector-size', type=int, default=64, metavar='L', help='Logic Vector size')
parser.add_argument('--logic-layer-num', type=int, default=3, metavar='L', help='Logic Layer Number')
...
parser.add_argument(
    '--logic-overshooting-distance',
    type=int,
    default=100,
    metavar='D',
    help='Logic Overshooting Distance',
)
parser.add_argument(
...
parser.add_argument('--model-learning-rate', type=float, default=1e-3, metavar='α', help='Learning rate')
parser.add_argument('--model-logic-learning-rate', type=float, default=6e-3, metavar='α', help='Learning Logic rate')
parser.add_argument('--logic-model-learning_rate', type=float, default=0.01, metavar='α', help='Logic_Learning rate')
...
parser.add_argument('--l2-weight', type=float, default=1e-5, metavar='L', help='L2 Weight')
parser.add_argument('--disclam', type=float, default=0.95, metavar='H', help='discount rate to compute return')
parser.add_argument('--optimisation-iters', type=int, default=10, metavar='I', help='Planning optimisation iterations')
parser.add_argument('--candidates', type=int, default=1000, metavar='J', help='Candidate samples per iteration')
parser.add_argument('--top-candidates', type=int, default=100, metavar='K', help='Number of top candidates to fit')
parser.add_argument('--test', action='store_true', help='Test only')
parser.add_argument('--test-img', action='store_true', help='Test only')
parser.add_argument('--test-interval', type=int, default=25, metavar='I', help='Test interval (episodes)')
parser.add_argument('--test-episodes', type=int, default=10, metavar='E', help='Number of test episodes')
parser.add_argument('--test-img-episodes', type=int, default=100, metavar='E', help='Number of test episodes')
...
logic_model = NLR(args.state_size, env.action_size, args.logic_vector_size, args.logic_layer_num,
                  args.logic_overshooting_distance).to(device=args.device)
logic_model._init_weights()
...
logic_model_optimizer = optim.SGD(
    logic_model.parameters(), lr=args.logic_model_learning_rate
)

# model_logic_optimizer = optim.SGD(
#     transition_model.parameters(), lr=args.model_logic_learning_rate
# )

model_logic_optimizer = optim.Adam(
    transition_model.parameters(), lr=args.model_logic_learning_rate, eps=args.adam_epsilon
)
...
if args.models != '' and os.path.exists(args.models):
    ...
    logic_model.load_state_dict(model_dicts['logic_model'])
...
if args.test_img:
    ...
    logic_loss = []
    with torch.no_grad():
        for _ in tqdm(range(args.test_img_episodes)):
            ...
            imagination_traj = imagine_ahead(
                posterior_states, beliefs, actor_model, transition_model, args.planning_horizon
            )

            imged_beliefs, imged_prior_states, imged_prior_means, imged_prior_std_devs, imged_actions = imagination_traj

            shape_prior = imged_prior_states[:-1].shape
            shape_actions = imged_actions[:-1].shape
            shape_next_prior = imged_prior_states[1:].shape

            imged_prior_states = torch.randn(shape_prior, device=imged_prior_states.device)
            imged_actions = torch.randn(shape_actions, device=imged_actions.device)
            imged_prior_states = torch.randn(shape_next_prior, device=imged_prior_states.device)

            pre_sim, r_loss, l2_loss = logic_model(imged_prior_states[:-1], imged_actions[:-1],
                                                   imged_prior_states[1:])

            logic_loss.append(pre_sim)

    logic_loss = torch.cat(logic_loss, dim=0)
    mean_pre_sim = torch.mean(logic_loss)
    std_pre_sim = torch.std(logic_loss)

    print(f"Mean of pre_sim: {mean_pre_sim}")
    print(f"Standard Deviation of pre_sim: {std_pre_sim}")
    env.close()
    quit()
...
if args.test:
    ...
    with torch.no_grad():
        ...
        init_belief, init_state = torch.zeros(args.batch_size, args.belief_size, device=args.device), torch.zeros(
            1, args.state_size, device=args.device
        )
        ...
        # Collect logic information
        # pre_sim = logic_model.test(imged_prior_states[:-1], imged_actions[:-1],
        #                                        imged_prior_states[1:])
        #
...
for episode in tqdm(
    ...
    for s in tqdm(range(args.collect_interval)):
        ...
        with FreezeParameters(logic_model.modules()):
            pre_sim, r_loss, l2_loss = logic_model(imged_prior_states[:-1], imged_actions[:-1],
                                                   imged_prior_states[1:])

        logic_loss = 1 - pre_sim.mean()

        # Update System 1 With Logical Signal (logic loss) From System 2
        model_loss = observation_loss + reward_loss + kl_loss + 10 * logic_loss
        ...
        del imagination_traj

        # Update System 2 With Logical Relationships in the Latent Space from System 1
        logic_posterior_states = posterior_states.detach()
        pre_sim, r_loss, l2_loss = logic_model(logic_posterior_states[:-1], actions[1:-1],
                                               logic_posterior_states[1:])
        logic_loss = 1 - pre_sim.mean() + r_loss + args.l2_weight * l2_loss
        logic_model_optimizer.zero_grad()
        logic_loss.backward()
        nn.utils.clip_grad_norm_(logic_model.parameters(), args.grad_clip_norm, norm_type=2)
        logic_model_optimizer.step()

        shape_prior = logic_posterior_states[:-1].shape
        shape_actions = actions[1:-1].shape
        shape_next_prior = logic_posterior_states[:-1].shape
        random_prior_states = torch.randn(shape_prior, device=imged_prior_states.device)
        random_actions = torch.randn(shape_actions, device=imged_actions.device)
        random_next_prior_states = torch.randn(shape_next_prior, device=imged_prior_states.device)
        contra_pre_sim, r_loss, l2_loss = logic_model(random_prior_states, random_actions, random_next_prior_states)
        logic_loss = contra_pre_sim.mean() + r_loss + args.l2_weight * l2_loss
        logic_model_optimizer.zero_grad()
        logic_loss.backward()
        nn.utils.clip_grad_norm_(logic_model.parameters(), args.grad_clip_norm, norm_type=2)
        logic_model_optimizer.step()
        ...
        imged_beliefs, imged_prior_states, imged_prior_means, imged_prior_std_devs, _ = imagination_traj
    ...
    metrics['logic_loss'].append(losses[5])
    lineplot(
    ...
    lineplot(metrics['episodes'][-len(metrics['logic_loss']):], metrics['logic_loss'], 'logic_loss', results_dir)
    ...
    writer.add_scalar("logic_loss", metrics['logic_loss'][0][-1], metrics['steps'][-1])
    print(
        ...
        torch.save(
            {
                ...
                'logic_model': logic_model.state_dict(),
                'model_optimizer': model_optimizer.state_dict(),
                'actor_optimizer': actor_optimizer.state_dict(),
                'value_optimizer': value_optimizer.state_dict(),
                'logic_optimizer': logic_model_optimizer.state_dict(),
            },
